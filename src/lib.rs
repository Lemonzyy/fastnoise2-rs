...
impl FastNoise {
    pub fn from_name(metadata_name: &str) -> Result<Self, String> {
        ...
        let handle = unsafe { ffi::fnNewFromMetadata(metadata_id, 0) };
        Ok(Self {
            handle,
            metadata_id,
        })
    ...
    pub fn from_encoded_node_tree(encoded_node_tree: &str) -> Option<Self> {
        ...
        let node_ptr = unsafe { ffi::fnNewFromEncodedNodeTree(cstring.as_ptr(), 0) };
        if node_ptr.is_null() {
            None
        } else {
            Some(Self {
                handle: node_ptr,
                metadata_id: unsafe { ffi::fnGetMetadataID(node_ptr) },
            })
        }
    }

    pub fn get_simd_level(&self) -> u32 {
        unsafe { ffi::fnGetSIMDLevel(self.handle) }
    }
    ...
    pub fn set(&self, member_name: &str, value: MemberValue) -> Result<(), String> {
        ...
        match value {
            MemberValue::Float(v) => match member.member_type {
                MemberType::Float => {
                    if !unsafe { ffi::fnSetVariableFloat(self.handle, member.index, v) } {
                        return Err("Failed to set float value".to_string());
                    }
                }
                MemberType::Hybrid => {
                    if !unsafe { ffi::fnSetHybridFloat(self.handle, member.index, v) } {
            ...
            MemberValue::Int(v) => match member.member_type {
                MemberType::Int => {
                    if !unsafe { ffi::fnSetVariableIntEnum(self.handle, member.index, v) } {
            ...
            MemberValue::Enum(v) => match member.member_type {
                MemberType::Enum => {
                    ...
                    if !unsafe { ffi::fnSetVariableIntEnum(self.handle, member.index, *enum_idx) } {
            ...
            MemberValue::NodeLookup(node) => match member.member_type {
                MemberType::NodeLookup => {
                    if !unsafe { ffi::fnSetNodeLookup(self.handle, member.index, node.handle) } {
                        return Err("Failed to set node lookup".to_string());
                    }
                }
                MemberType::Hybrid => {
                    if !unsafe {
                        ffi::fnSetHybridNodeLookup(self.handle, member.index, node.handle)
                    } {
    ...
    pub fn gen_single_2d(&self, x: f32, y: f32, seed: i32) -> f32 {
        unsafe { ffi::fnGenSingle2D(self.handle, x, y, seed) }
    }

    pub fn gen_single_3d(&self, x: f32, y: f32, z: f32, seed: i32) -> f32 {
        unsafe { ffi::fnGenSingle3D(self.handle, x, y, z, seed) }
    }

    pub fn gen_single_4d(&self, x: f32, y: f32, z: f32, w: f32, seed: i32) -> f32 {
        unsafe { ffi::fnGenSingle4D(self.handle, x, y, z, w, seed) }
    }
}

impl Drop for FastNoise {
    fn drop(&mut self) {
        unsafe { ffi::fnDeleteNodeRef(self.handle) };
    }
}
